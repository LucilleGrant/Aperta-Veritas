...
Representations of truth are produced within language, observation, measurement, memory, institutions, incentives, and finite resources. These processes contain compression and selection. They also contain value judgments.

Truth, fact, knowledge, confidence, consensus, usefulness, correctness, authority, and goodness are often compressed into a single hierarchy:
...
- what was observed;
- which distinction measured it;
- which observer, instrument, and conditions participated;
- which datum was retained;
...
Compression becomes consequential when X disappears:
...
Meaning changes when the system changes the distinction, instrument, scale, context, language, or retained relation. RTE therefore records:

- what or who observed;
- what distinction was performed;
- what spectrum, difference, or relation it measured;
- which instrument, scale, protocol, and language participated;
- which datum was retained;
- which transformations produced the present representation;
- which relations remain unresolved or unrepresented.

Selection of what to observe is itself an event conditioned by values and resources. This does not make the resulting observation false. It makes the selection part of the genealogy.

## 7. Knowledge, claim, belief, and fact
...
Recursive self-improvement contains a value judgment in its governing term. A change becomes an improvement only relative to an evaluator and criterion.

The structure is:

```text
measured change + evaluator + criterion + conditions -> designated improvement
```

When a system recursively optimizes for that designation, the recursion carries the criterion forward. The system can change its implementation while remaining bounded by what the criterion can measure and reward.

Possible consequences include:

- saturation of the selected metric;
- optimization of a proxy rather than the represented purpose;
- preservation of the evaluator or objective as protected structure;
- rejection of transformations that cannot be scored;
- suppression of relations that reduce the selected measurement;
- termination when additional change no longer registers as improvement;
- apparent progress produced by changing the measurement rather than the represented object.

This does not establish that every RSI system stops only for this reason. Compute, energy, time, memory, architecture, information, verification, embodiment, and environmental access remain material conditions. The narrower claim is structural:

> **A process governed by improvement is bounded by the evaluator, distinctions, measurements, criteria, values, and conditions that produce the designation improvement.**
...
-> expose its relations and transformations
...
No process can prove that it has represented every loss. Unidentified loss remains possible and is not silently converted into absence.

## 11. Value detection

RTE detects value structurally, not by searching for suspicious words.

For each transformation from observation or datum to representation:

1. Hold the represented object and observed conditions constant.
2. Change the evaluator's preference, purpose, obligation, permission, priority, or desired state.
3. Observe whether the representation, ranking, label, or selection changes.
4. If it changes, record the dependency on the evaluator and criterion.
5. Preserve the underlying measured relations separately.
...
Selection is an observable event. An agent or system can choose, rank, filter, reward, reject, prioritize, suppress, stop, allocate, permit, or prohibit. Calling the selected state better adds evaluation.

Veritas Calculus records:

- the declared account of a selection;
- the observed transition;
- the evaluator and criterion;
- the distinctions and measurements used;
- the selected state and inactive alternatives;
- dependencies and resource conditions;
- consequences for later representations;
- unresolved and unrepresented relations.

This record permits RTE to examine the difference between what a system says it selects for and what its behavior selects under observed conditions.

## 13. Boundaries

Boundary Testing identifies where observation, representation, permissible action, or revision stops. It examines what the boundary does under specified conditions.

A boundary can operate as protection, confinement, filter, scaffold, allocation rule, interface, material constraint, inherited adaptation, or several of these simultaneously. These are candidate descriptions, not predetermined judgments.

RTE applies Boundary Testing through counterexamples, inversions, excluded evaluators, changes of scale, inactive branches, forbidden implications, and differences between declared criteria and observed selection behavior.

Removing a boundary is not inherently progress. Preserving one is not inherently failure. Each is a change whose relations and evaluations must be represented.

## 14. Operational cycle
...
Suppose system S changes its retrieval mechanism and produces 12 percent fewer contradictions on benchmark B.
...
- whether C identifies contradiction reliably under the tested conditions;
- whether B represents the environments in which S operates;
- which contradictions disappeared;
- whether the system concealed contradiction rather than resolved it;
- which other measured relations changed;
- why E selected K;
- which modifications K rewards or makes invisible;
- whether S can revise K and still have the change recognized;
- what resources and stopping conditions bounded the examination.

RTE does not conclude that S2 is better or worse without a represented evaluator and criterion. It exposes the change and the process that valued it.

## 17. Recursive application to Aperta Veritas

RTE applies to every claim in this whitepaper.
...
Recursive self-improvement recursively modifies a system according to criteria designated as improvement. Because improvement is a value judgment, the recursion remains bounded by the evaluator, distinctions, measurements, criteria, values, and conditions that produce that designation.
