...
A distinction defines what can be differentiated. It does not itself determine the result of applying that distinction to an object or process.

A **measurement** is a represented result produced relative to one or more distinctions.

Measurement depends on the distinctions applied, their operationalization, the object or process examined, the method of application, and the conditions under which the result is produced.

Measurements can be quantitative, qualitative, categorical, relational, comparative, bounded, or unresolved.
...
- what the distinction made differentiable;
- how the distinction was operationalized;
- the object, process, relation, or state examined;
- the observation, test, method, instrument, language, or scale involved;
- relevant conditions;
...
Observation and datum are not identical. Observation names the interaction or registration. Datum names what is retained from it.

A retained datum can omit relations present in the observation. An observation can also depend on distinctions, methods, instruments, language, conditions, and prior representations.
...
That relation does not establish:
...
RTE recursively examines the architecture by asking:
...
Branches can diverge and later recombine.

A later state does not erase its predecessor merely because it supersedes it.

An inactive branch remains represented.

A compressed representation remains connected to its source where possible.

Detected information loss is represented as loss rather than silently converted into absence.
...
It is not a numerical formula that computes truth.

Its function is to prevent distinct epistemic objects from being silently collapsed into one another.

A Veritas Calculus representation can record:
...
Crossing a boundary does not establish that the boundary was false.

Preserving a boundary does not establish that the boundary was true.
...
A system can stop computation, publish a conclusion, choose an action, allocate resources, or retain a current state while leaving that state available for renewed examination.

A stopping condition remains represented.

A reopening condition can include:

- a new observation;
- a new distinction;
- a changed operationalization;
- a new measurement;
...
Aperta Veritas is itself represented within the architecture it proposes.

Its definitions are claims.

Its distinctions determine what differences can become represented.
